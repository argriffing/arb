...
void
_acb_hypgeom_gamma_lower_series(acb_ptr g, const acb_t s, acb_srcptr h, slong hlen, slong n, slong prec)
{
    acb_t c;
    acb_init(c);

    acb_hypgeom_gamma_lower_series(c, s, h, 0, prec);
...
void
acb_hypgeom_gamma_lower_series(acb_poly_t g, const acb_t s, const acb_poly_t h, slong n, slong prec)
{
    slong hlen = h->length;
    ...
    if (hlen == 0)
    {
        acb_t t;
        acb_init(t);
        _acb_hypgeom_gamma_lower_series(g->coeffs, s, t, 1, n, prec);
        acb_clear(t);
    }
    else
    {
        _acb_hypgeom_gamma_lower_series(g->coeffs, s, h->coeffs, hlen, n, prec);
    }
